...
void G4d2oCylindricalDetector::Initialize(){
    ...
    if (pmtDiameter > 0.0) {
      ...
      h2oHeight = d2oHeight + 2*(acrylicEndCapThickness + h2oTCthickness + pmtMinorAxis/2.0 + pmtLegLength/2.0); //z //added /2.0 to pmts!!!!!
    }
...
G4LogicalVolume * G4d2oCylindricalDetector::GetDetector(){
    ...
    G4VPhysicalVolume *teflonPhysV = new G4PVPlacement(0,G4ThreeVector(0,0,-(pmtLegLength+pmtMinorAxis-h2oTCthickness+2.0*teflonThickness)/2.0),teflonLining,"teflonPhysV",h2oLogV,false,0,true); // added -(pmtLegLength+pmtMinorAxis-h2oTCthickness+teflonThickness)/2.0!!!!
...
G4LogicalVolume *G4d2oCylindricalDetector::GetTeflonLiningLogV(){
  //d2oHeight + 2*(acrylicThickness + h2oTCthickness + pmtMinorAxis + pmtLegLength);
    ...
    G4double teflonheight = (d2oHeight/2.0) + acrylicEndCapThickness + h2oTCthickness + (pmtMinorAxis/2.0) - (pmtLegLength+3.0*teflonThickness)/2.0; //added - (pmtLegLength+2.0*teflonThickness)/2.0 !!!!
