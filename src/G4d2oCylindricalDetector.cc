...
G4LogicalVolume * G4d2oCylindricalDetector::GetDetector(){
    ...
    ///// AIR BUBBLE /////
    G4LogicalVolume *bubbleLogV = GetbubbleLogV();
    ...
    ///// Place the Bubble in D2O tank /////
    //new G4PVPlacement(0,G4ThreeVector(0,0,(d2oHeight-bubbleHeight)/2.0),bubbleLogV,"bubblePhysV",d2oLogV,false,0,true);
    ...
    //new G4PVPlacement(0,G4ThreeVector(0,0,-(pmtLegLength+pmtMinorAxis)/2.0),acrylicLogV,"acrylicPhysV",h2oLogV,false,0,true); // added -(pmtLegLength+pmtMinorAxis)/2.0!!!!
    new G4PVPlacement(0,G4ThreeVector(0,0,-h2oTCthickness/2.0),acrylicLogV,"acrylicPhysV",h2oLogV,false,0,true);
    ...
    if (teflonCapBot) {
      if (pmtDiameter < 0.0) 
       // teflonCapBotPhysV = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, -(h2oHeight-teflonThickness)/2.0 ), teflonCapBot, "teflonCapBotPhysV", h2oLogV, false, 0, true);//Bring back when prudent!!!!!
        teflonCapBotPhysV = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, -(h2oHeight-teflonThickness-0.5*pmtLegLength-0.5*pmtMinorAxis)/2.0 ), teflonCapBot, "teflonCapBotPhysV", h2oLogV, false, 0, true); // FOR H2O ONLY, COMMENT THIS IN. [For  H2O up to PMT hemisphere]
      else 
       // teflonCapBotPhysV = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, -(h2oHeight-teflonThickness)/2.0), teflonCapBot, "teflonCapBotPhysV", h2oLogV, false, 0, true);//Bring back when prudent!!!!!
        teflonCapBotPhysV = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, -(h2oHeight-teflonThickness-0.5*pmtLegLength-0.5*pmtMinorAxis)/2.0), teflonCapBot, "teflonCapBotPhysV", h2oLogV, false, 0, true); // FOR H2O ONLY, COMMENT THIS IN. [For H2O up to PMT hemisphere]
...
G4LogicalVolume *G4d2oCylindricalDetector::GetH2OLogV(){
    ...
    //G4Tubs* h2oSolid = new G4Tubs("h2oSolid",0.0,(h2oLength)/2.0,(h2oHeight)/2.0,0.0,360.0*deg); // Bring this back this when appropiate!!!. [H2O up to the end of PMTS]
    
    G4Tubs* h2oSolid = new G4Tubs("h2oSolid",0.0,(h2oLength)/2.0,(h2oHeight-0.5*pmtLegLength-0.5*pmtMinorAxis)/2.0,0.0,360.0*deg); // FOR H2O ONLY, COMMENT THIS BACK IN. [H2O up to PMT hemisphere]
...
G4LogicalVolume *G4d2oCylindricalDetector::GetbubbleLogV(){
    
    G4Tubs* bubbleSolid = new G4Tubs("bubbleSolid",0.0,bubbleRadius/2.0,bubbleHeight/2.0,0.0,360.0*deg);
    G4LogicalVolume *bubbleLogV = new G4LogicalVolume(bubbleSolid, 
                                                   matPtr->GetMaterial( AIR ),
                                                   "bubbleLogV");
    G4VisAttributes *visAttbubble = new G4VisAttributes();
    visAttbubble->SetColour(G4Color::Gray());
    visAttbubble->SetForceSolid(true);
    bubbleLogV->SetVisAttributes( visAttbubble );
