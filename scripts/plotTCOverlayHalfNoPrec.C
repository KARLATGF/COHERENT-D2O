...
std::vector<std::string> fnames = {"data/prodTC5cm/Sim_D2ODetector00*.root",
  "data/prodTC10HalfSize2/Sim_D2ODetector00*.root",
...
//    Sim_Tree->Draw(Form("numHits>>%s",hNameO.Data()),"WeightO()*(vol0>0)","",200000);
    ...
    while(ievent < Sim_Tree -> GetEntries()){
...
void LoadHists(){
  ...
  if(logBinning){
    ...
    fIn = TFile::Open("TCStudyHalfV2.root");
  }
...
//  TH1* hO = vhistOProj[ifav]->DrawCopy("C NOHIST SAME");
  ...
  hD->GetYaxis()->SetTitle("Counts/MeV (0.67t D_{2}O) 4 SNS#bulletYears");
...
//        }
//    }
//    std::cout<<"Precision Minimum:"<<PrecisionMin<<" at Energy Cut "<<PrecisionE<<std::endl;
...
//    
  auto hOxygenAsSignal = (TH1*)hO->Clone("hOxygenAsSignal");
    ...
    hOxygenAsSignal->GetYaxis()->SetTitle("Counts/MeV (0.67t D_{2}O) 4 SNS#bulletYears");
