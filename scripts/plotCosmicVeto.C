...
void plotCosmicVeto()
{
    ...
    double simDuration = 32.341; //data/MuonVetoTest2//Sim_D2ODetector002.root
    ...
    std::string fname = "data/MuonVetoTest2/Sim_D2ODetector002.root";
    ...
    TH1* hLY = new TH1D(hNameLY.Data(),
                        hNameLY.Data(),
                        100,0,100);
    TString hNameLYMuonCut(Form("hLY_CosmicMuonCut"));
    TH1* hLYMuonCut = new TH1D(hNameLYMuonCut.Data(),
                               hNameLYMuonCut.Data(),
                               100,0,100);
    ...
    while(t.Next()&&ievent<maxevents){
        ...
        if(ev->NumVetoPairs(5.0)>0)
            hLYMuonCut->Fill(ev->numHits/pePerMeV);
    }
    ...
    hLY->Scale(1.0,"width");
    hLYMuonCut->Scale(totalLiveTimeSNSYear/simDuration);
    hLYMuonCut->Scale(1.0,"width");
    ...
    hLY->SetLineColor(kBlue);
    hLY->SetLineColor(kRed);
    
    new TCanvas("c1");

    hLY->Draw();
    hLYMuonCut->Draw("same");
    
    new TCanvas("c1");
    auto hMuonEfficiency = (TH1*) hLYMuonCut->Clone("hMuonEfficiency");
    hMuonEfficiency->Divide(hLY);
    hMuonEfficiency->Draw();
